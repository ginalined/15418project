...
__global__ void split_cuda(double *all_box, int *t, tri *cuda_tris, int N,
  ...
  if (j >= N)
    return;
  //int maxThread = N < (blockIdx.x * (blockDim.x + 1)) ? N : (blockIdx.x * (blockDim.x + 1));
...
VCInternal::VCInternal(int mySize, int ss) {
  ...
  cudaMalloc(&cuda_trans, sizeof(double) * mySize * 17);
...
int VCInternal::UpdateAllTrans(int id[], int total, double *trans) {
  ...
  cudaMemcpy(cuda_trans, trans, sizeof(double) * total * 17, cudaMemcpyHostToDevice);

  cuda_update_trans<<<32, 32>>>(total, cuda_trans, cuda_boxes);
  cudaDeviceSynchronize();
...
// __global__ cuda_collide(int N, int * overlaps){
//   int index = blockIdx.x + blockDim.x + threadIdx.x;
//   if (index > N)
//     return;
//   // int val = dev[k];
